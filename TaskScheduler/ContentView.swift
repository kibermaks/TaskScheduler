...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: Notification.Name("PresetsUpdated"))) { _ in
            appState.presets = PresetStorage.shared.loadPresets()
            if let current = appState.selectedPreset,
               let refreshed = appState.presets.first(where: { $0.id == current.id }) {
                appState.selectedPreset = refreshed
            }
        }
        .onReceive(updateService.$pendingAlert) { alert in
            appState.updateAlert = alert
        }
        .alert(item: $appState.updateAlert, content: buildUpdateAlert)
    }
}
...
struct ContentViewBody: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $appState.showingNewPresetSheet) {
        ...
        .alert("Schedule Sessions", isPresented: $appState.showingConfirmation) {
            Button("Cancel", role: .cancel) { }
            Button("Schedule") { appState.scheduleAllSessions() }
        } message: {
            Text("Add \(schedulingEngine.projectedSessions.count) sessions to your calendar?")
        }
        .sheet(isPresented: $appState.showingDeleteConfirmation) {
            DeleteConfirmationSheet(onDelete: appState.deleteScheduledSessions)
                .environmentObject(schedulingEngine)
        ...
        .sheet(isPresented: $appState.showingPatternsGuide) {
            PatternsGuide()
        }
        .sheet(isPresented: $appState.showingTasksGuide) {
            TasksGuide()
        }
        ...
        .onReceive(NotificationCenter.default.publisher(for: Notification.Name("ShowTasksGuide"))) { _ in
            appState.showingTasksGuide = true
        }
        .task { await appState.requestCalendarAccess() }
        .onAppear { appState.onAppearActions() }
        .onDisappear { appState.nowTimer?.invalidate() }
        .onChange(of: appState.dateSelection) { _, newSelection in
            appState.updateSelectedDate(for: newSelection)
        }
        .onChange(of: appState.selectedDate) { _, newDate in
            Task {
                if appState.dateSelection == .custom {
                    let calendar = Calendar.current
                    let hour = calendar.component(.hour, from: appState.startTime)
                    let minute = calendar.component(.minute, from: appState.startTime)
                    if let updatedStartTime = calendar.date(bySettingHour: hour, minute: minute, second: 0, of: newDate) {
                        appState.startTime = updatedStartTime
                    }
                }

                await calendarService.fetchEvents(for: newDate)
                if appState.autoPreview { appState.updateProjectedSchedule() }
            }
        }
        .onChange(of: appState.startTime) { _, _ in
            if appState.autoPreview && !appState.useNowTime { appState.updateProjectedSchedule() }
        }
        .onChange(of: appState.useNowTime) { _, isNow in
            if isNow { appState.startTime = appState.roundedNowTime() }
            if appState.autoPreview { appState.updateProjectedSchedule() }
        }
        ...
        .onReceive(NotificationCenter.default.publisher(for: NSApplication.didBecomeActiveNotification)) { _ in
            ...
            if appState.dateSelection == .today {
                if !calendar.isDate(appState.selectedDate, inSameDayAs: Date()) {
                    appState.updateSelectedDate(for: .today)
                    dateChanged = true
                }
            } else if appState.dateSelection == .tomorrow {
                if let tomorrow = calendar.date(byAdding: .day, value: 1, to: Date()),
                   !calendar.isDate(appState.selectedDate, inSameDayAs: tomorrow) {
                    appState.updateSelectedDate(for: .tomorrow)
                    dateChanged = true
                }
            }

            if !dateChanged {
                if appState.useNowTime && appState.dateSelection == .today {
                    appState.startTime = appState.roundedNowTime()
                }

                Task {
                    await calendarService.fetchEvents(for: appState.selectedDate)
                    if appState.autoPreview { appState.updateProjectedSchedule() }
                }
            }
        }
    ...
    private var mainHStack: some View {
        HStack(spacing: 0) {
            ...
            TimelineView(selectedDate: appState.selectedDate, startTime: appState.effectiveStartTime)
                .padding()
...
struct HeaderView: View {
    ...
    private var dateButtons: some View {
        HStack(spacing: 12) {
            ForEach(DateSelection.allCases, id: \.self) { sel in
                Button { appState.dateSelection = sel } label: {
                    Text(sel.rawValue)
                        .font(.system(size: 14, weight: .medium))
                        .padding(.horizontal, 16)
                        .padding(.vertical, 8)
                        .background(appState.dateSelection == sel ? Color(hex: "8B5CF6") : Color.white.opacity(0.1))
                        .foregroundColor(.white)
                        .cornerRadius(8)
                }
                .buttonStyle(.plain)
    ...
    private var startTimeControls: some View {
        HStack(spacing: 8) {
            ...
            Picker("", selection: $appState.useNowTime) {
                Text("Now").tag(true)
                Text("Set").tag(false)
            }
            ...
            ZStack {
                if !appState.useNowTime {
                    DatePicker("", selection: $appState.startTime, displayedComponents: .hourAndMinute)
                        .labelsHidden()
                        .frame(width: 85)
                        .overlay(
...
struct LeftPanel: View {
    ...
    private var presetMenu: some View {
        Menu {
            ForEach(appState.presets) { preset in
                Button { appState.applyPreset(preset) } label: { Label(preset.name, systemImage: preset.icon) }
            }
            Divider()
            Button { appState.showingNewPresetSheet = true } label: { Label("Save Current as New Preset...", systemImage: "plus") }
            if let p = appState.selectedPreset {
    ...
    private func updateCurrentPreset(_ preset: Preset) {
        ...
        appState.presets = PresetStorage.shared.loadPresets()
        appState.selectedPreset = updated
    }

    private func deletePreset(_ preset: Preset) {
        PresetStorage.shared.deletePreset(preset)
        appState.presets = PresetStorage.shared.loadPresets()
        appState.selectedPreset = appState.presets.first
        if let first = appState.selectedPreset {
            appState.applyPreset(first)
        }
    }
}
...
struct RightPanel: View {
    ...
    private var availabilityCard: some View {
        let avail = schedulingEngine.calculateAvailability(startTime: appState.effectiveStartTime, baseDate: appState.selectedDate, busySlots: calendarService.busySlots)
    ...
    private var actionButtons: some View {
        let planningExists = calendarService.hasPlanningSession(for: appState.selectedDate)
        let primaryRed = Color(hex: "EF4444")
        ...
        return VStack(spacing: 12) {
            ...
            Button(action: { appState.showingConfirmation = true }) {
            ...
            Button(action: { appState.deletePastSessions = false; appState.showingDeleteConfirmation = true }) {
    ...
    private func createPlanningSession() {
        if let session = schedulingEngine.projectSingleSession(
            type: .planning,
            startTime: appState.effectiveStartTime,
            baseDate: appState.selectedDate,
...
struct DeleteConfirmationSheet: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Toggle(isOn: $appState.deletePastSessions) {
            ...
            HStack(spacing: 16) {
                Button("Cancel") { appState.showingDeleteConfirmation = false }
                    .keyboardShortcut(.cancelAction)

                Button("Delete Events") { onDelete(); appState.showingDeleteConfirmation = false }
